...
function ChooseCard({ id, active, onClick }: { id: string; active: boolean; onClick: () => void }) {
  ...
  return <button type="button" className={`service-card choose-card ${active ? 'is-active' : ''}`} aria-expanded={active} aria-pressed={active} onClick={onClick}>
    ...
    <div className="service-card-foot"><span>{service.features[0]}</span><span className="choose-card-toggle"><span>{count} 項解決方案</span><ChevronDown size={18} /></span></div>
  </button>;
}
...
export default function ChooseService() {
  ...
  const findDirection = () => {
    const message = selectedOptions.length ? `我感興趣的解決方案方向：\n${selectedOptions.map(option => `• ${option.title}`).join('\n')}\n\n（請補充你的業務背景與最想解決的問題）` : undefined;
    openConsult({ message });
  };
  return <main id="main" className="choose-page">
    <section className="choose-hero"><div className="container"><nav className="breadcrumbs" aria-label="麵包屑導覽"><Link href="/">首頁</Link><span aria-hidden="true">/</span><span aria-current="page">選擇我們的解決方案</span></nav><p className="hero-announcement"><span className="online-dot" />OUR EXPERTISE</p><h1>你的挑戰，<br />我們的出發點。</h1><p className="catalog-lead">先選你感興趣的方向，我們一起找到適合的起步方式。點選左邊任一類別展開解決方案項目，可全選或逐項挑選，再交給我們建議方向。</p></div></section>
    <section className="section choose-body"><div className="container">
      <div className="choose-layout">
        <div className="choose-cards" aria-label="解決方案類別">
        ...
        <aside className="choose-panel" aria-live="polite">
          {activeGroup ? <>
            <div className="choose-panel-head"><div><span className="mini-label">SOLUTION OPTIONS</span><h2>{services.find(item => item.id === activeGroup.cardId)?.title}</h2></div><button type="button" className="text-button" onClick={() => selectAllInGroup(activeGroup)}>全選此類別</button></div>
            ...
            <p className="choose-panel-note"><CircleDot size={14} />勾選感興趣的項目，稍後會一併帶入諮詢摘要。點選箭頭可查看每項解決方案詳情。</p>
          </> : <div className="choose-panel-empty"><span className="empty-glyph"><Sparkles size={24} strokeWidth={1.4} /></span><h2>點選左邊任一解決方案類別</h2><p>就會在這裡展開可選取的解決方案項目。你隨時可以回到首頁查看「OUR EXPERTISE」的完整介紹。</p></div>}
        </aside>
      </div>
      <div className="choose-summary">
        <div className="choose-summary-count"><span className="summary-badge">{selectedOptions.length}</span><span>已選取 {selectedOptions.length} 項解決方案</span>{selectedOptions.length === 0 && <em>尚未選取任何項目</em>}</div>
