...
function ServicesMegaMenu({ pathname, close }: { pathname: string; close: () => void }) {
  ...
  return <div className="services-mega" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
    <button ref={triggerRef} className={`nav-trigger ${servicesActive ? 'active' : ''}`} aria-expanded={open} aria-controls="services-mega-panel" onClick={() => setOpen(value => !value)}>解決方案<ChevronDown size={13} /></button>
    <div id="services-mega-panel" className={`services-mega-panel ${open ? 'is-open' : ''}`} hidden={!open} aria-label="解決方案選單">
      <div className="mega-panel-inner">
        <div className="mega-left">
          <p className="mega-heading">解決方案分類<span>SOLUTION CATEGORIES</span></p>
          ...
          <Link href="/catalog" className="mega-all" onClick={close}>查看全部 50 個方案<ArrowUpRight size={15} /></Link>
          <Link href="/choose-service" className="mega-choose" onClick={close}>選擇我們的解決方案<ArrowUpRight size={15} /></Link>
        </div>
...
function Header() {
  ...
  return <header ref={header} className="site-header multipage-header"><div className="container header-inner"><Brand /><nav className="desktop-nav page-navigation" aria-label="主選單"><Link href="/" aria-current={current('/')} className={pathname === '/' ? 'active' : ''} onClick={closeMenus}>首頁</Link><details className="solution-dropdown" ref={dropdown}><summary className={pathname.startsWith('/solutions/') ? 'current-solution' : ''}>服務對象<ChevronDown size={13} /></summary><div className="solution-menu"><p>找到適合你的起點<span>WHO WE SERVE</span></p>{solutionLinks.map(item => <Link key={item.href} href={item.href} aria-current={current(item.href)} onClick={closeMenus}><span className="nav-solution-icon"><item.icon size={20} strokeWidth={1.5} /></span><span><strong>{item.label}</strong><small>{item.note}</small></span><ArrowUpRight size={15} /></Link>)}<div className="solution-menu-foot">不同規模，同樣從你的問題出發。</div></div></details><Link href="/catalog" aria-current={current('/catalog')} className={pathname === '/catalog' ? 'active' : ''} onClick={closeMenus}>解決方案</Link><Link href="/cases" aria-current={current('/cases')} className={pathname === '/cases' ? 'active' : ''} onClick={closeMenus}>真實案例</Link><Link href="/about" aria-current={current('/about')} className={pathname === '/about' ? 'active' : ''} onClick={closeMenus}>關於我們</Link><Link href="/contact" aria-current={current('/contact')} className={pathname === '/contact' ? 'active' : ''} onClick={closeMenus}>聯絡我們</Link></nav><div className="header-actions"><ConsultButton className="header-consult nav-booking" /><button ref={mobileButton} disabled={!ready} className="menu-toggle icon-button" aria-label={mobileOpen ? '關閉選單' : '開啟選單'} aria-expanded={mobileOpen} aria-controls="mobile-menu" onClick={() => setMobileOpen(!mobileOpen)}>{mobileOpen ? <X /> : <Menu />}</button></div></div><nav id="mobile-menu" className={`mobile-nav multipage-mobile ${mobileOpen ? 'is-open' : ''}`} aria-label="流動版選單" inert={!mobileOpen}><Link href="/" aria-current={current('/')} onClick={closeMenus}>首頁<ArrowUpRight size={15} /></Link><p className="mobile-group-label">服務對象 <span>WHO WE SERVE</span></p>{solutionLinks.map(item => <Link key={item.href} href={item.href} aria-current={current(item.href)} onClick={closeMenus}><span><item.icon size={16} />{item.label}</span><ArrowUpRight size={15} /></Link>)}<p className="mobile-group-label">解決方案 <span>SOLUTIONS</span></p><Link href="/catalog" aria-current={current('/catalog')} onClick={closeMenus}>解決方案<ArrowUpRight size={15} /></Link><Link href="/choose-service" aria-current={current('/choose-service')} onClick={closeMenus}>選擇我們的解決方案<ArrowUpRight size={15} /></Link>{categories.map(category => <details className="mobile-category" key={category.slug}><summary><span className="mobile-category-icon">{(() => { const Icon = categoryIcons[category.icon]; return <Icon size={15} strokeWidth={1.5} />; })()}</span>{category.title}<ChevronDown size={14} /></summary><div className="mobile-category-list"><Link href={`/services/${category.slug}`} onClick={closeMenus}>查看{category.title}全部</Link>{category.services.map(service => <Link key={service.slug} href={`/services/${category.slug}/${service.slug}`} onClick={closeMenus}>{service.title}</Link>)}</div></details>)}<p className="mobile-group-label">認識實瀚 <span>ABOUT</span></p><Link href="/about" aria-current={current('/about')} onClick={closeMenus}>關於我們<ArrowUpRight size={15} /></Link><Link href="/cases" aria-current={current('/cases')} onClick={closeMenus}>真實案例<ArrowUpRight size={15} /></Link><Link href="/contact" aria-current={current('/contact')} onClick={closeMenus}>聯絡我們<ArrowUpRight size={15} /></Link><div className="mobile-consult-row"><ConsultButton className="button button-primary mobile-consult" /></div></nav><noscript><nav className="noscript-navigation" aria-label="無 JavaScript 導覽"><Link href="/">首頁</Link><Link href="/solutions/solo">一人公司</Link><Link href="/solutions/sme">中小企業</Link><Link href="/solutions/enterprise">企業團隊</Link><Link href="/services">解決方案</Link><Link href="/choose-service">選擇我們的解決方案</Link></nav></noscript></header>;
}

function Footer() {
  return <footer className="site-footer"><div className="container"><div className="footer-top"><div className="footer-brand"><Brand /><p>讓科技，成為你的成長力。</p><span>香港 · 專注企業級數位化解決方案</span></div><div className="footer-column"><h3>服務對象</h3>{solutionLinks.map(item => <Link key={item.href} href={item.href}>{item.label}</Link>)}</div><div className="footer-column"><h3>解決方案與合作</h3><Link href="/services">解決方案</Link><Link href="/choose-service">選擇我們的解決方案</Link><Link href="/services#delivery">交付與支援</Link><Link href="/solutions/sme#engagement">合作方式</Link></div><div className="footer-column"><h3>認識實瀚</h3><Link href="/about">關於我們</Link><Link href="/cases">真實案例</Link><Link href="/contact">聯絡我們</Link><Link href="/services#faq">常見問題</Link><ConsultButton className="footer-contact" /></div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} 實瀚科技 Hangs Technology. All rights reserved.</span><span className="footer-motto"><span className="online-dot" />BUILT WITH PURPOSE.</span><a href="#main">回到頂部<ArrowUpRight size={13} /></a></div></div></footer>;
}
